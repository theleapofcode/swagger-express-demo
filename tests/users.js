...
Test('/users', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         */
        t.test('test findUsers get operation', function (t) {
            Mockgen().requests({
                path: '/users',
                operation: 'get'
            }, function (err, mock) {
         ...
         * responses: 200, default
         */
        t.test('test addUser post operation', function (t) {
            Mockgen().requests({
                path: '/users',
                operation: 'post'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/users']['post']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
